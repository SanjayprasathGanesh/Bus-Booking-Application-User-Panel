...
class VolvoMultiAxleBus extends StatefulWidget {
  final String? busId;
  final String uEmail;
  const VolvoMultiAxleBus({Key? key, required this.busId, required this.uEmail}) : super(key: key);

  @override
  State<VolvoMultiAxleBus> createState() => _VolvoMultiAxleBusState();
}

class _VolvoMultiAxleBusState extends State<VolvoMultiAxleBus> {
  ...
  Widget buildLastSeatIconButton(int rowIndex, int seatIndex) {
    int seatNumber = 48 + seatIndex; // Start seat numbers from 37
                  ...
                  ElevatedButton(
                    ...
                    onPressed: (){
                      if(selectedSeats.isNotEmpty){
                        Navigator.push(context, MaterialPageRoute(builder: (context) => UserDetails(docId: widget.busId!, seats: selectedSeats, uEmail: widget.uEmail,)));
                      }
                      else{
                        ToastContext().init(context);
                        Toast.show(
